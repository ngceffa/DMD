...
class dmd():
    ...
    def project(self, pattern, value=0.1):
        ...
        self.power.constant(value)
    
    def project_continuous_square_signal(self, pattern, frequency, V_max, V_min):
        """Project an image.
        Can accept path to image or directly a numpy array."""
        if type(pattern) == str:
            mask = imageio.imread(pattern)
    ...
    def project_finite_square_signal(self, pattern, frequency, V_max, V_min, reps=1):
        """Project an image.
        Can accept path to image or directly a numpy array."""
        if type(pattern) == str:
            mask = imageio.imread(pattern)
        ...
        self.power.square_finite(frequency, V_max, V_min, reps=reps)

    def project_continuous_ramp_plus_delay(self, pattern, frequency, duty, V_max, V_min):
        """Project an image.
        Can accept path to image or directly a numpy array."""
        if type(pattern) == str:
            mask = imageio.imread(pattern)
    ...
    def project_random_slopes_seqence(self, pattern, value, protocol_name):
        """ Pseudo random
            - value = max power for DMD laser in V
            - 5s per phase
            1- raising phase
        ...
        self.power.randomised_building_blocks(value, protocol_name=protocol_name)
